        ...
        return random.nextInt(MAX_FIGURE_SIZE);
         ...
         public Figure whichFigure() {
              switch (Figures.values()[random.nextInt(Figures.values().length)]) {
                  ...
                  case RIGHTTRIANGLE:
                      return new RightTriangle(colorSupplier.getRandomColor(), figureSize(), figureSize());
                  default: ISOSCELESTRAPEZOID:
                      return new IsoscelesTrapezoid(colorSupplier.getRandomColor(), figureSize(), figureSize(), figureSize());
