...
public class FigureSupplier {
    ...
    public Figure getRandomFigure() {
        switch (random.nextInt(NUMBER_OF_FIGURES)) {
            ...
            case 4:
                return new IsoscelesTrapezoid(colourSupplier.getRandomColour(), random.nextInt(MAX_LENGTHS)
                , random.nextInt(MAX_LENGTHS), random.nextInt(MAX_LENGTHS));
            default:
                return new Square(colourSupplier.getRandomColour(), random.nextInt(MAX_LENGTHS));
        }
    }
