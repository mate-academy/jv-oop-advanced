...
All figures have 
- **state** - all figures have `string`, but each figure type can also have one or several unique properties (`radius` for circle, `firstLeg` and `secondLeg` for right triangle, and so on).
...
```
 Figure: square, area: 25.0 sq.units, side: 5 units, string: blue
 Figure: triangle, area: 12.5 sq.units, firstLeg: 7 units, secondLeg: 5 units, string: yellow
```
