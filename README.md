...
All figures have 
- **state** - all figures have `color`, but each figureObject type can also have one or several unique properties (`radius` for circle, `firstLeg` and `secondLeg` for right triangle, and so on).
- **behaviour** - we can obtain the area of any figureObject and are able to draw it. To 'draw' means to print out all information about a figureObject using `System.out.println()` (you shouldn't override toString() method for this). 
...
- and `FigureSupplier` with `public Figure getRandomFigure()` method - for generating figureObject with random properties.
