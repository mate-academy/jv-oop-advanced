...
Let's generate random values in `core.basesyntax.Main.FigureSupplier`.
...
* Bad example:
```
public class core.basesyntax.Main.FigureSupplier {
...
#### Don't extend your `Main/Application` class from `core.basesyntax.Main.FigureSupplier` or `ColorSupplier`.
To be able to call the non-static method, we just need to create an instance of the class: 
```
core.basesyntax.Main.FigureSupplier figureSupplier = new core.basesyntax.Main.FigureSupplier();
Figure randomFigure = figureSupplier.getRandomFigure();
```

#### You should create several random Figures, so you will use a loop. Please don't create a `new core.basesyntax.Main.FigureSupplier()` inside the loop.
